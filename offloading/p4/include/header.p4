...
header zenoh_header_t{
    bit<3> flags;
    bit<5> msgId;
}


header zenoh_attachment_t{
    bit<8> len;
    varbit<1024> payload;
}


header zenoh_byte_elem_t{
    bit<8> elem;
}


header zenoh_serial_number_t{
    bit<80> number;
}
/*
header zenoh_attachment_len_t{
    bit<8> len;
}

header zenoh_attachment_payload_t{
    varbit<1024> payload;
}
*/
#ifdef WITH_SPGW
...
struct fabric_metadata_t {
    ...
    bit<80>    zenoh_serial_number;
    bit<4>        zenoh_byte_counter;
    zenoh_byte_elem_t[10] byte_stack;
    bit<8> zenoh_resourceId;
    _BOOL isZenoh;
    _BOOL isZenohData;
    _BOOL isZenohOpen;
    _BOOL isZenohOpenAck;
    _BOOL applyACL;
...
struct parsed_headers_t {
    ...
    udp_t udp;
    icmp_t icmp;
    zenoh_header_t zenoh_header;
    zenoh_attachment_t zenoh_attachment;
    zenoh_header_t zenoh_inner_header;
    zenoh_byte_elem_t zenoh_byte_elem;
    zenoh_open_t zenoh_open;
    zenoh_serial_number_t zenoh_serial_number;
    zenoh_byte_elem_t[10] byte_stack;
    //zenoh_attachment_len_t zenoh_attachment_len;
    //zenoh_attachment_payload_t zenoh_attachment_payload;
    //zenoh_frame_t zenoh_frame;
